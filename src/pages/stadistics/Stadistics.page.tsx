...
export const StadisticsPage = () => {
    ...
    const calcAnalytics = (c: Campaign) => Math.floor(Math.random() * (c.budget * 1.5));
    const calcVisits = (c: Campaign) => Math.floor(calcAnalytics(c) * 0.8);
    const calcEarnings = (c: Campaign) => parseFloat(((calcVisits(c) / 1000) * c.reward).toFixed(2));
    const calcCPM = (c: Campaign) => parseFloat(((calcEarnings(c) / calcVisits(c)) * 1000).toFixed(2));
    ...
    <div className="h-full w-full bg-zinc-900 flex flex-col justify-start py-8 px-4 sm:px-6 lg:px-8">
         <div>
            ...
            <p className="text-white/70 mb-6">
                Obtenes métricas de tu contenido.
            </p>
        </div>
        {loading ? <Spinner/> : <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1 gap-6 w-2/3">
            {myCampaings.map(c => (
