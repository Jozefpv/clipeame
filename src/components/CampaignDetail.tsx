...
export const CampaignDetail: React.FC<CampaignDetailProps> = ({
  ...
  const overallProgress = Math.min(
    100,
    ((budget - budgetRemaining) / budget) * 100,
  );
  const [isModalOpen, setModalOpen] = useState(false);
  console.log(files, paid);
  return (
    <>
            ...
            <div className="px-4 py-3 overflow-x-auto">
              ...
              <table className="min-w-full table-auto text-white">
                ...
                <tbody className="divide-y divide-white/20">
                  {participants &&
                    participants.map(({ postLink, views }, idx) => (
                      <tr
                        key={idx}
                        className="hover:bg-blue-500/30 cursor-pointer"
                        onClick={() => window.open(postLink, '_blank')}
                      >
                        <td className="px-3 py-2 truncate">Video</td>
                  ...
                  {participants.length === 0 && (
                    <tr>
                      <td
                        colSpan={2}
                        className="px-3 py-2 text-center text-white/50"
                      >
                        Sin participaciones
                      </td>
                    </tr>
            ...
            <div className="p-6 flex-1 space-y-6 overflow-auto">
              ...
              <div className="bg-yellow-400/30 text-zinc-900 px-4 py-3 rounded-full text-sm">
                <p className="font-medium text-yellow-100/100">
                  ⓘ Envía tu post para que lo revisen en el plazo de 1 hora
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-6 text-sm">
                <div className="col-span-2">
                  ...
                  <p className="font-bold text-white">
                    {(budget - budgetRemaining).toFixed(2)}€ de{' '}
                    {budget.toFixed(0)}€
                  </p>
                ...
                <div>
                  ...
                  <p className="font-bold text-white">
                    {maxPayment.toFixed(0)}€
                  </p>
                </div>
                <div>
                  <p className="font-medium text-white/70">CATEGORÍA</p>
                  <p className="font-bold text-white">
                    {mapCampaignCategory(categoryId)}
                  </p>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-lg">
                  {mapCampaignSocialmedia(socialMediaId).includes(
                    'YOUTUBE',
                  ) && <FaYoutube className="w-5 h-5" />}
                  {mapCampaignSocialmedia(socialMediaId).includes('TIKTOK') && (
              ...
              <div>
                ...
                <p className="text-sm text-white">
                  Sube tu clip de {authorName} bajo tu propia responsabilidad.
                  Asegúrate de cumplir con todos los requisitos de calidad y
                  derechos de imagen.
                </p>
              </div>
            </div>
